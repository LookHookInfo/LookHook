...
export const useXroleReward = () => {
  ...
  const queries = useMemo(() => {
    return [
      ...
      },
      {
        ...
        queryFn: () => readContract({ contract: hashcoinContract, method: 'balanceOf', params: [xroleRewardContract.address] }),
        enabled: !!account,
  ...
  const claimMutation = useMutation({
    mutationFn: async () => {
      ...
      const tx = prepareContractCall({ contract: xroleRewardContract,
        method: 'claim',
        params: [],
      });
  ...
  const formattedPoolRewardBalance = poolRewardBalance ? parseFloat(formatUnits(poolRewardBalance, 18)).toLocaleString() : '0';
