...
import { gramroleRewardContract, hashcoinContract } from '../utils/contracts';
...
export const useTelegramReward = () => {
  ...
  const queries = useMemo(() => {
    return [
      {
        queryKey: ['gramrole', 'canClaim', accountAddress],
        queryFn: async () => {
          if (!accountAddress) return false;
          return await xPublicClient.readContract({
            address: gramroleRewardContract.address as `0x${string}`,
            abi: xroleRewardAbi,
      ...
      {
        queryKey: ['gramrole', 'poolBalance', gramroleRewardContract.address],
        queryFn: async () => {
          return await publicClient.readContract({
            address: hashcoinContract.address as `0x${string}`,
            abi: erc20Abi,
            functionName: 'balanceOf',
            args: [gramroleRewardContract.address as `0x${string}`],
          });
        },
        enabled: true,
        staleTime: 900000,
      ...
      {
        queryKey: ['gramrole', 'hasClaimed', accountAddress],
        queryFn: async () => {
          if (!accountAddress) return false;
          return await xPublicClient.readContract({
            address: gramroleRewardContract.address as `0x${string}`,
  ...
  const claimMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['gramrole'] });
    },
    onError: (error: Error) => {
