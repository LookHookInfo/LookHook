...
const ALCHEMY_CAN_RPC_URL = import.meta.env.VITE_ALCHEMY_CAN_RPC_URL;
...
if (!ALCHEMY_CAN_RPC_URL) {
  throw new Error('Missing VITE_ALCHEMY_CAN_RPC_URL environment variable');
}
...
// Клиент для Early Bird & Whale (EarlyBird, WhaleContract)
...
// Клиент для CAN (HashMinerBurnRedeem)
export const canPublicClient = createPublicClient({
  chain: base,
  transport: http(ALCHEMY_CAN_RPC_URL),
  batch: {
    multicall: true,
  },
});
