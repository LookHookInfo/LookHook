...
export function DolphinAchievement({ wallet }: WhaleAchievementProps) {
  ...
  const { data: whaleContractData, isLoading: isLoadingWhaleContract, refetch: refetchWhaleContract } = useReadContract({
  ...
  return (
    ...
    >
      <img src="/assets/Dolphin.webp" alt="Dolphin Achievement" className={`size-10 ${!hasNft && !canMint ? 'opacity-50' : ''}`} />
      {canMint && !hasNft && (
...
export function SharkAchievement({ wallet }: WhaleAchievementProps) {
  ...
  return (
    ...
    >
      <img src="/assets/Shark.webp" alt="Shark Achievement" className={`size-10 ${!hasNft && !canMint ? 'opacity-50' : ''}`} />
      {canMint && !hasNft && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 text-white text-xs font-bold">
          CLAIM
        </div>
...
export function WhaleAchievement({ wallet }: WhaleAchievementProps) {
  ...
  return (
    ...
    >
      <img src="/assets/Whale.webp" alt="Whale Achievement" className={`size-10 ${!hasNft && !canMint ? 'opacity-50' : ''}`} />
      {canMint && !hasNft && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 text-white text-xs font-bold">
          CLAIM
        </div>
