...
function TierDisplay({ tierId, tierName, stakeData, amount, refreshBalances, refetchAllowance, isApproved, apr, tokenSymbol, setStatus, showQuestTooltip }: TierDisplayProps) {
    ...
    return (
        <div className="bg-neutral-900 rounded-lg p-4 flex flex-col justify-between w-full">
            <div>
                ...
                <p className="text-sm text-neutral-400">Staked: {(stakeData.amount / BigInt(10 ** 18)).toString()} {tokenSymbol}</p>
                <p className="text-sm text-neutral-400">Remaining: {isStaked ? formatRemainingTime(stakeData.timeLeft) : "N/A"}</p>
            </div>
            <div className="mt-4">
                {isStaked ? (
                    ...
                    <div className="flex justify-center">
                                            <button onClick={handleStaking} disabled={isProcessing || !amount || Number(amount) <= 0} className="btn-claim">
...
export default function UserStakesDisplay({ userStakes, apr3M, apr6M, apr12M, amount, tokenSymbol, refreshBalances, refetchAllowance, isApproved, status, setStatus }: UserStakesDisplayProps) {
    let s3 = { amount: 0n, rewards: 0n, timeLeft: 0n };
    let s6 = { amount: 0n, rewards: 0n, timeLeft: 0n };
    let s12 = { amount: 0n, rewards: 0n, timeLeft: 0n };

    if (userStakes) {
        s3 = { amount: BigInt(userStakes[0] || 0n), rewards: BigInt(userStakes[1] || 0n), timeLeft: BigInt(userStakes[2] || 0n) };
        s6 = { amount: BigInt(userStakes[3] || 0n), rewards: BigInt(userStakes[4] || 0n), timeLeft: BigInt(userStakes[5] || 0n) };
        s12 = { amount: BigInt(userStakes[6] || 0n), rewards: BigInt(userStakes[7] || 0n), timeLeft: BigInt(userStakes[8] || 0n) };
    }

    return (
        <div className="flex flex-col sm:flex-row gap-3">
            <TierDisplay tierId={1} tierName="3 Month" stakeData={s3} apr={apr3M} amount={amount} tokenSymbol={tokenSymbol} refreshBalances={refreshBalances} refetchAllowance={refetchAllowance} isApproved={isApproved} status={status} setStatus={setStatus} />
            <TierDisplay tierId={2} tierName="6 Month" stakeData={s6} apr={apr6M} amount={amount} tokenSymbol={tokenSymbol} refreshBalances={refreshBalances} refetchAllowance={refetchAllowance} isApproved={isApproved} status={status} setStatus={setStatus} />
            <TierDisplay tierId={3} tierName="12 Month" stakeData={s12} apr={apr12M} amount={amount} tokenSymbol={tokenSymbol} refreshBalances={refreshBalances} refetchAllowance={refetchAllowance} isApproved={isApproved} status={status} setStatus={setStatus} showQuestTooltip={true} />
        </div>
    );
}