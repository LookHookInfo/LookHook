...
### Успешная стратегия оптимизации RPC (на примере Drub-блока)

**Принцип:** Использование `@tanstack/react-query` для батчинга запросов и тонкой настройки кэширования.

**Ключевые шаги:**
1.  **Централизованный `QueryClientProvider`:** Убедиться, что `QueryClientProvider` из `@tanstack/react-query` обертывает все приложение (обычно в `src/main.tsx` или `src/App.tsx`) для предоставления `QueryClient` всему дереву компонентов.
2.  **Использование `useQueries` для батчинга:**
    *   Внутри кастомного хука (например, `useDrubContract.ts`), импортировать `useQueries` и `useQueryClient`.
    *   Создать вспомогательную функцию (`createThirdwebQuery`), которая генерирует объекты запросов для `useQueries`. Она должна включать:
        *   `queryKey`: Массив, содержащий `contract.address`, имя метода и параметры.
        *   `queryFn`: Функция, вызывающая `readContract` из `thirdweb` с соответствующими параметрами.
        *   `enabled`: Условие для выполнения запроса (например, `!!account`).
        *   `staleTime`: Время (в мс), в течение которого данные считаются "свежими" и не требуют повторного запроса.
        *   `refetchInterval`: Время (в мс), через которое данные будут обновляться в фоновом режиме.
    *   Сформировать массив `queries` с помощью `useMemo` из всех необходимых запросов.
    *   Вызвать `useQueries({ queries })` и деструктурировать результаты в соответствующие переменные.
3.  **Гранулярная инвалидация кэша с `queryClient.invalidateQueries()`:**
    *   После каждой транзакции (например, `unifiedBuy`, `addLiquidity`, `burnAllPositions`), использовать `queryClient.invalidateQueries()` для инвалидации *только тех конкретных запросов*, которые могли измениться.
    *   Использовать `queryKey` с `contract.address`, именем метода и (при необходимости) конкретными параметрами (например, `accountAddress`) и `exact: true` для максимальной точности.
    *   Это предотвращает избыточные RPC запросы, которые происходят при инвалидации всего контракта.
4.  **Удаление ручных `refetch` и `useEffect`:** Убрать все ручные вызовы `refetch` и `useEffect` с `setInterval` для обновления данных, так как `staleTime` и `refetchInterval` от `@tanstack/react-query` берут на себя эту ответственность, делая это более эффективно.
5.  **Консолидация импортов:** Убедиться, что все необходимые импорты (`readContract`, `ThirdwebContract`, `Abi`, `QueryClient`, `QueryClientProvider`) присутствуют, а неиспользуемые удалены.
...
8.  [DONE] Коммит изменений для Drub-блока.
...
**План:** Применить "Успешную стратегию оптимизации RPC", описанную выше, к GMNFT-блоку, сосредоточившись на файле `src/hooks/useGMNFTContract.ts`. (Но **без использования `git` команд и без модификации самого `useGMNFTContract.ts`**, так как это будет делать пользователь.)
